...
from ..utils import Config, traceError, print_func_name
import logging
...
class Architecture:
    ...
    @print_func_name
    def __init__(self, model_name=Config.env_config['DEFAULT_AI_MODEL'], temperature=Config.env_config['DEFAULT_AI_TEMPERATURE'], instructions=Config.env_config['DEFAULT_AI_INSTRUCTIONS']):
    ...
    def invoke(self, input: dict) -> dict:
        ...
        try:
            if Config.langfuse_handler:
                response = self.agent.invoke(input=input, config={"callbacks": [Config.langfuse_handler]})
    ...
    async def ainvoke(self, input: dict) -> dict:
        ...
        try:
            if Config.langfuse_handler:
                response = await self.agent.ainvoke(input=input, config={"callbacks": [Config.langfuse_handler]})
...
class ContentWriterArchitecture(Architecture):
    ...
    def __init__(self, 
                 ...
                 temperature=Config.env_config['DEFAULT_AI_TEMPERATURE'], 
                 instructions=Config.env_config['DEFAULT_AI_INSTRUCTIONS'], 
                 type='base', 
                 collection_name='', 
...
class OutlineCreatorArchitecture(Architecture):
    ...
    @print_func_name
    def __init__(self, model_name=Config.env_config['DEFAULT_AI_MODEL'], temperature=Config.env_config['DEFAULT_AI_TEMPERATURE'], instructions=Config.env_config['DEFAULT_AI_INSTRUCTIONS'], dir_path_ref_files=None):
