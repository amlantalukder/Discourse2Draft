...
class AnalyzeContentHeaderSchema(BaseModel):
    '''
    Represents the list of keyphrases extracted from content header
    to get context on.
    '''
...
class AnalyzeContentHeader:
    ...
    analyze_content_header_human_prompt = '''
    ...
    def __init__(self, llm, for_lit_search=False):
        ...
        if not for_lit_search:
            max_keyphrases = Config.MAX_KEYPHRASES
            obj_schema = AnalyzeContentHeaderSchema
        else:
            max_keyphrases = Config.MAX_KEYPHRASES_LIT_SEARCH
            obj_schema = AnalyzeContentHeaderSchemaForLitSearch

        parser = parser = OutputFixingParser.from_llm(parser=PydanticOutputParser(pydantic_object=obj_schema), llm=llm)
        self.analyze_content_header_prompt = setPrompt(self.analyze_content_header_system_prompt(max_keyphrases), 
                                              self.analyze_content_header_human_prompt, 
                                              parser)
        self.analyze_content_header_chain = self.analyze_content_header_prompt | llm | parser
    ...
    def __call__(self, state: State) -> State:
        ...
        response = self.analyze_content_header_chain.invoke(
            {
        ...
        return {'keyphrases': response, 'steps': ['analyze_content_header']}